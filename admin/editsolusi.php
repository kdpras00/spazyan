...
if(isset($_POST['ubah'])){
    ...
    if(empty($id_penyakit) || empty($solusi)) {
        ...
        $sql = "UPDATE tbl_solusi SET id_penyakit = ?, solusi = ? WHERE id_solusi = ?";
        $stmt = mysqli_prepare($koneksi, $sql);
        mysqli_stmt_bind_param($stmt, "ssi", $id_penyakit, $solusi, $id_solusi);

        if(mysqli_stmt_execute($stmt)){
            $_SESSION['pesan_sukses'] = "Data solusi berhasil diperbarui.";
...
$id_edit = $_GET['id_solusi'];

$sql_select = "SELECT * FROM tbl_solusi WHERE id_solusi = ?";
$stmt_select = mysqli_prepare($koneksi, $sql_select);
mysqli_stmt_bind_param($stmt_select, "i", $id_edit);
...
// Mengambil daftar penyakit untuk dropdown
...
<head>
    ...
    <title>Edit Data Solusi - Admin</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Form Edit Data Solusi</h1>
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form method="POST" action="editsolusi.php?id_solusi=<?= htmlspecialchars($d['id_solusi']); ?>">
                                <input type="hidden" name="id_solusi" value="<?= htmlspecialchars($d['id_solusi']); ?>">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="id_penyakit" name="id_penyakit" required>
                                        <?php while($p = mysqli_fetch_array($penyakit_query)): ?>
                                            <option value="<?= htmlspecialchars($p['id_penyakit']) ?>" <?= ($p['id_penyakit'] == $d['id_penyakit']) ? 'selected' : '' ?>>
                                                <?= htmlspecialchars($p['id_penyakit']) ?> - <?= htmlspecialchars($p['nama_penyakit']) ?>
                                            </option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
